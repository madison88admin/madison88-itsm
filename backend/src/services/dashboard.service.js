...
const DashboardService = {
    ...
    async getAdvancedReporting() {
        // Consolidate smaller queries to save connections (avoid MaxClientsInSessionMode)
        const [
            mttrRaw,
            mttaResult,
            reopenStats,
            trendData, // Combined volume and SLA breaches
            performanceData, // Combined agent, team, and priority stats
            approvalsAndEscalations, // Combined metadata counts
            agentStatusMatrix
        ] = await Promise.all([
            db.query(`SELECT created_at, resolved_at, location FROM tickets WHERE resolved_at IS NOT NULL`),
            db.query(`
            ...
            // Combined Trends
            db.query(`
                (SELECT 'ticket_volume' as type, DATE_TRUNC('day', created_at)::date AS day, COUNT(*)::int AS count
            ...
            db.query(`
                WITH priority_stats AS (
                    SELECT priority, COUNT(*)::int AS total,
                           SUM(CASE WHEN sla_due_date IS NOT NULL AND sla_due_date < NOW() AND status NOT IN ('Resolved','Closed') THEN 1 ELSE 0 END)::int AS breached
                    FROM tickets GROUP BY priority
                ),
                weekly_stats AS (
                    SELECT DATE_TRUNC('week', created_at)::date AS week_start, COUNT(*)::int AS total,
                           SUM(CASE WHEN sla_due_date IS NOT NULL AND sla_due_date < NOW() AND status NOT IN ('Resolved','Closed') THEN 1 ELSE 0 END)::int AS breached
                    FROM tickets GROUP BY week_start ORDER BY week_start DESC LIMIT 12
                )
                SELECT 'priority' as type, priority as key, total, breached, NULL as week_start FROM priority_stats
                UNION ALL
                SELECT 'weekly' as type, NULL as key, total, breached, week_start FROM weekly_stats
            `),
            // Combined Metadata Counts
            db.query(`
                SELECT
                    (SELECT COUNT(*)::int FROM change_requests WHERE status = 'submitted') as change_requests,
                    (SELECT COUNT(*)::int FROM ticket_priority_override_requests r JOIN tickets t ON t.ticket_id = r.ticket_id WHERE r.status = 'pending') as priority_overrides,
                    (SELECT COUNT(*)::int FROM tickets t JOIN sla_rules s ON s.priority = t.priority AND s.is_active = true WHERE t.status NOT IN ('Resolved','Closed') AND t.sla_due_date IS NOT NULL AND t.created_at IS NOT NULL AND EXTRACT(EPOCH FROM (NOW() - t.created_at)) / NULLIF(EXTRACT(EPOCH FROM (t.sla_due_date - t.created_at)), 0) * 100 >= s.escalation_threshold_percent) as escalations,
                    (SELECT
                      JSON_BUILD_OBJECT(
                        'bucket_0_1', SUM(CASE WHEN created_at >= NOW() - INTERVAL '1 day' THEN 1 ELSE 0 END)::int,
                        'bucket_2_3', SUM(CASE WHEN created_at < NOW() - INTERVAL '1 day' AND created_at >= NOW() - INTERVAL '3 days' THEN 1 ELSE 0 END)::int,
                        'bucket_4_7', SUM(CASE WHEN created_at < NOW() - INTERVAL '3 days' AND created_at >= NOW() - INTERVAL '7 days' THEN 1 ELSE 0 END)::int,
                        'bucket_8_14', SUM(CASE WHEN created_at < NOW() - INTERVAL '7 days' AND created_at >= NOW() - INTERVAL '14 days' THEN 1 ELSE 0 END)::int,
                        'bucket_15_plus', SUM(CASE WHEN created_at < NOW() - INTERVAL '14 days' THEN 1 ELSE 0 END)::int
                      )
                     FROM tickets WHERE status NOT IN ('Resolved','Closed')) as aging
            `),
            db.query(`
        ...
        // Secondary parallel queries for things that depend on previous results (if any)
        const [agentPerf, slaByTeam, agentWorkload, topTags] = await Promise.all([
            db.query(`
                SELECT u.user_id, u.full_name,
        ...
        // Process MTTR
        const businessMinutes = mttrRaw.rows.map(row => {
        ...
        const mttr_hours = mttrRaw.rows.length > 0 ? Number((totalBusinessHours / mttrRaw.rows.length).toFixed(2)) : 0;
        ...
        const sla_compliance_by_week = performanceData.rows
        ...
        // Process Consolidated Metadata
        const metadata = approvalsAndEscalations.rows[0] || {};
        ...
        return {
            ...
            approvals_pending: {
                change_requests: metadata.change_requests || 0,
                priority_overrides: metadata.priority_overrides || 0,
            },
            escalations_open: metadata.escalations || 0,
            aging_buckets: metadata.aging || {
                bucket_0_1: 0,
                bucket_2_3: 0,
                bucket_4_7: 0,
                bucket_8_14: 0,
